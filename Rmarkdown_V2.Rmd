...
Therefore, variable associated with wind direction are removed and associated with 4 qualititative variable each one associated with a given wind direction. An intermediate mean_direction is used. The mean_direction takes the means directions taken at different heights.
...
The following R code is the one used to automatize the choise of the model. Comments are added so as to describe better the purpose of each instruction.
...
All variables retained are significant while some modalities of Month "V1" are not. Therefore, we will check the sign of coefficients with the actual breackdown of tomorrow raining state per month. 
```{r, echo = FALSE}
mosaicplot(d0$Month~d2$V38, xlab = "Month", ylab = "Raining tomorrow", main = "Breakdown per month")
```

The main conclusion is that the coeffecient are consistant. Indeed, January is the month during which there is the highest proportion of rainy days compared to other months. 
Coefficient of the other variables and could be check trough boxplots. 
...
```{r, echo = FALSE}
mosaicplot(td0$Month~td0$Pluie.demain, xlab = "Month", ylab = "Raining tomorrow", main = "Breakdown per month")
```

By comparing the plot above with the one associated with training data, roughly the same behavior is observed but not exactly the same since here June in the predicted to the rainiest month while in the training data June is the third rainiest month.
Finally, the model that we have could does not have a high accuracy level but provided acceptable results. 
